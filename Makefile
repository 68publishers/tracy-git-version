...
stan:
	PHP=74 make composer.update
	docker exec 68publishers.tracy-git-version-panel.74 vendor/bin/phpstan analyse --level 7 src

coverage:
	PHP=81 make composer.update
	docker exec 68publishers.tracy-git-version-panel.81 vendor/bin/tester -p phpdbg -C -s --coverage ./coverage.xml --coverage-src ./src ./tests
...
ifndef PHP
	...
	docker exec 68publishers.tracy-git-version-panel.$(PHP) composer update --no-progress --prefer-dist --prefer-stable --optimize-autoloader --quiet
...
ifndef PHP
	...
	docker exec 68publishers.tracy-git-version-panel.$(PHP) composer update --no-progress --prefer-dist --prefer-lowest --prefer-stable --optimize-autoloader --quiet
...
ifndef PHP
	...
	docker exec 68publishers.tracy-git-version-panel.$(PHP) vendor/bin/tester -C -s ./tests
